...
describe("test construction", () => {
    it("should fail to create cookie", () => {
        ...
        expect(() => Cookie.from(new Uint8Array(9)).toString()).to.throw(
            Error,
            `malformed cookie option: invalid length 9`
        );
        expect(() => Cookie.from(new Uint8Array(41)).toString()).to.throw(
            Error,
    ...
    it("should create cookie from data", () => {
        ...
        expect(
            Cookie.from([0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf]).toString()
